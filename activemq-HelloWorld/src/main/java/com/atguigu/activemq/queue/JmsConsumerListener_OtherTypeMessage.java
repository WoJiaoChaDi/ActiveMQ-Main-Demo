...
public class JmsConsumerListener_OtherTypeMessage {
    ...
    public static void main(String[] args) throws JMSException, IOException {
        ...
        //需要在客户端限制
        //或者 修改服务器配置
        activeMQConnectionFactory.setTrustedPackages(new ArrayList<>(Arrays.asList("com.atguigu.activemq.queue")));
        ...
        messageConsumer.setMessageListener((message) -> {
            if(null != message && message instanceof MapMessage){
                ...
                try {
                    System.out.println("***消费者-消息监听-收到Map消息1***： " + mapMessage.getString("k1"));
                    System.out.println("***消费者-消息监听-收到Map消息2***： " + mapMessage.getBoolean("boolean"));
                    System.out.println("***消费者-消息监听-收到Map消息3***： " + mapMessage.getObject("list"));
                    System.out.println("***消费者-消息监听-收到Map消息4***： " + mapMessage.getObject("person"));
                } catch (JMSException e) {
                    e.printStackTrace();
                }
            }
            if(null != message && message instanceof ObjectMessage){
                ...
                try {
                    ...
                    System.out.println("***消费者-消息监听-收到Obj消息1***： " + person.toString());
                } catch (JMSException e) {
                    e.printStackTrace();
                }
            }
            if(null != message && message instanceof  TextMessage){
                TextMessage textMessage = (TextMessage) message;
                try {
                    System.out.println("***消费者-消息监听-收到Text消息***： " + textMessage.getText());
                } catch (JMSException e) {
                    e.printStackTrace();
                }
            }
