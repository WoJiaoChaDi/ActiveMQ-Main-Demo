...
public class JmsProduceMapMessage {
    ...
    public static void main(String[] args) throws JMSException {
        ...
        for (int i = 0; i < 1; i++) {
            ...
            list.add("3");
            list.add("4");
            mapMessage.setObject("list", list);

            //只能用List Map String 类型的，不能用其他Object类型，如果需要用其他类型，应该用 ObjectMessage
            //Person person = new Person("张三", 12);
            //mapMessage.setObject("person", (Serializable) person);


            //设置消息的额外参数，也可以在 messageProducer.send方法中进行设置
            //设置消息的目的地，可以重新设置目的地
            mapMessage.setJMSDestination(queue);
            //设置消息的持久性
            mapMessage.setJMSDeliveryMode(DeliveryMode.PERSISTENT);
            //消息的过期时间（默认永不过期）
            mapMessage.setJMSExpiration(10000L);
            //消息的优先级，0-9 十个级别， 0-4 普通消息  5-9 加急消息
            mapMessage.setJMSPriority(9);
            //消息的id，唯一识别编号
            mapMessage.setJMSMessageID("MsgId_" + i);
